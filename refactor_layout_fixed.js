...
function processDir(dir) {
    ...
    for (const f of files) {
        ...
        if (fs.statSync(p).isDirectory()) {
            ...
            const returnRegex = /return\s*\(\s*<div className=\{[^}]+\.layoutWrapper[^}]+\}>[\s\S]*?<DashboardHeader[\s\S]*?\/>\s*/m;
            ...
            content = content.replace(/const \[mobileSidebarOpen[^;]+;\r?\n?/g, "");
            content = content.replace(/const \[activeNav[^;]+;\r?\n?/g, "");
            content = content.replace(/import DashboardHeader[^;]+;\r?\n?/g, "");
            content = content.replace(/import LogoutButton[^;]+;\r?\n?/g, "");
