...
// 1. Replace the headerActions Skeleton block
const headerActionsRegex = /<div className=\{styles\.headerActions\}>[\s\S]*?<div className=\{styles\.avatar\}[^>]*>[\s\S]*?<SkeletonBlock[^>]*>[\s\S]*?<\/div>\s*<\/div>/;
...
c = c.replace(headerActionsRegex, headerActionsReplacement);

// 2. Replace the mobileProfile Skeleton block
const mobileProfileRegex = /<div className=\{styles\.mobileProfile\}[^>]*>[\s\S]*?<SkeletonBlock[^>]*>[\s\S]*?<\/div>/;
...
c = c.replace(mobileProfileRegex, mobileProfileReplacement);
