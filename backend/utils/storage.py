...
for Supabase Storage buckets. Falls back to local filesystem storage
when Supabase is not configured, so the app stays functional in dev.
"""
import os
import uuid
import logging
...
def _local_upload(file_obj, prefix: str) -> dict:
    """Save file locally under BASE_DIR/media/ for dev use."""
...
def upload_file(file_obj, prefix: str = "uploads", bucket: str | None = None) -> dict:
    ...
    Falls back to local filesystem when Supabase is not configured (dev mode).

    Returns:
        {"key": str, "url": str, "public_url": str|None, "size": int, "content_type": str}
    """
    if not is_configured():
        logger.info("Supabase not configured; using local file storage")
        return _local_upload(file_obj, prefix)
