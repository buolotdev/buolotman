...
def zip_backend():
    ...
    with zipfile.ZipFile(zip_path, 'w', zipfile.ZIP_DEFLATED) as zipf:
        for root, dirs, files in os.walk(backend_dir):
            if 'venv' in root or '__pycache__' in root or '.git' in root or '.next' in root or '_backups_' in root:
                continue
                
            for file in files:
                if file.endswith('.sqlite3-journal') or file.endswith('.db-journal'):
                    continue
                ...
                elif linux_path.endswith('.sqlite3') or linux_path.endswith('.db'):
                    zinfo.external_attr = 0o666 << 16  # rw-rw-rw-
                else:
